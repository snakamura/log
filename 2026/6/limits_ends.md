...
Imagine that there is a discrete category $J$ with two objects and there are two functors from $J$ to $C$; constant functor $Delta(V)$ which maps both objects to $V$ in $C$, and functor $D$ that maps objects in $J$ to $P$ and $Q$ in $C$ respectively. [The limit of $D$ is $P \otimes Q$](../5/limits_adjunctions.html), which is `(P, Q)` if $C$ is $Hask$.
...
But what can we do with `a -> a`? It's neither a covariant functor nor a contravariant functor. It's when [a profunctor comes into play](../4/profunctors_ends.html). As we saw in the post, the end of profunctor `a -> a` is `forall a. a -> a`.
