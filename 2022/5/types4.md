...
As you can see, it takes `f` which is a cotainer of a value. In our case, it'll be `SSubCat`. We need it to write a function to check if a specified value is included in a list using propositional equality because [`TestEquality`](https://hackage.haskell.org/package/base-4.16.1.0/docs/Data-Type-Equality.html#t:TestEquality) works on a type indexed by a type.
