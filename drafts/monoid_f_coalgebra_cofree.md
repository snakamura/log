...
Just like an initial F-Algebra defines a free structure, a terminal F-Coalgebra defines a cofree structure which is a maximum structure of monoid. For example, when you have `3`, we can expand it to an infinite size of monoid like `(0 <> (0 <> (... <> 1))) <> ((0 <> (... <> 1)) <> (0 <> (... <> 1)))` by prepending its unit (`0`) to each non-unit value.
