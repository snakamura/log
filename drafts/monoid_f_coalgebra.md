...
This coalgebra build a monoid by dividing a number so that their sum becomes the number. You can build a monoid like this with it.
...
`sumMonoidIntCoalg` split each value in `Append` into two by applying `sumIntCoalg`.
...
`sumMonoidMonoidIntCoalg` again splits eah value in `Append` into two.
...
Just like F-Algebra defines a free structure, F-Coalgebra defines a cofree structure which is a maximum structure of monoid. For example, when you have `3`, we can expand it to an infinite size of monoid like `((0 <> (0 <> 0) <> 1) <> ....)`.

We can say that we can build an infinite structure using F-Coalgebra.
