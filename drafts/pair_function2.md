...
As we saw in the previous post, `Env` comonad allows you to get a single value. `Traced` comonad extends `Env` comonad by allowing you to get a value indexed by `m`. And, but you can move an index only by using `Monoid`.
